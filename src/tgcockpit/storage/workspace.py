...
Месячный план постов: дата · день · столп · тема · статус.
Статусы: идея → план → черновик → в очереди → вышел. ⚠ «нужна твоя история» — личный пост, пишется из рассказа автора.
Столпы и слот — из config.yaml. Заполняется через `/content-plan`.

## Текущий месяц

| Дата | День | Столп | Тема | Статус |
|------|------|-------|------|--------|
| — | — | — | (пусто — заполни через /content-plan) | идея |

## Бэклог идей (без даты)
- (темы без конкретной даты)
"""
