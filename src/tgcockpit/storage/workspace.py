...
Очередь обучающих постов (эксплейнеры, разборы): дата · день · тема · статус.
Статусы: идея → план → черновик → в очереди → вышел. Слот/ритм — из config.yaml. Заполняется через `/content-plan`.
Личные истории и анонсы тут НЕ планируются — их автор приносит сам, по мере появления.

## Текущий месяц

| Дата | День | Тема | Статус |
|------|------|------|--------|
| — | — | (пусто — заполни через /content-plan) | идея |

## Бэклог идей (без даты)
- (обучающие темы без конкретной даты)
"""
